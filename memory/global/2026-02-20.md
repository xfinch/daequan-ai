...
### MongoDB + Real-Time Architecture — Added

**Foundation built to avoid reconstructing database later:**

| Component | Purpose |
|-----------|---------|
| **MongoDB** (Railway) | Decision log, user state, session storage |
| **Change Streams** | Watch for DB changes, push to clients |
| **Socket.io** | Real-time WebSocket connections |
| **Mongoose** | Document modeling + queries |
| **connect-mongo** | Session persistence in MongoDB |

**Decision Schema:**
```javascript
{
  type: 'task' | 'alert' | 'insight' | 'action',
  title: String,
  description: String,
...
**Real-Time Flow:**
1. Decision made (background) → Saved to MongoDB
2. Socket.io broadcasts `decision:new` event
3. Dashboard receives event, animates new decision into UI
4. No page refresh needed

**API Endpoints:**
- `GET /api/decisions` — Query decisions (with filters)
- `POST /api/decisions` — Create new decision (broadcasts live)
- `PATCH /api/decisions/:id` — Update decision (broadcasts live)

### Railway Deployment Status
- MongoDB deployed: `balanced-miracle` project
- Public URL: `crossover.proxy.rlwy.net:19554`
- Express app ready for deployment
